...
import { ClipboardCheck, Clock, FileText, User, AlertTriangle } from 'lucide-react';
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="space-y-6 p-6">
                <Heading title="Assigned Reviews" description="Portfolios assigned to you for evaluation" />

                {assignments.data.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-lg border border-dashed py-12">
                        <ClipboardCheck className="text-muted-foreground mb-4 h-12 w-12" />
                        <p className="text-muted-foreground text-sm">No portfolios have been assigned to you yet.</p>
                    </div>
                ) : (
                    <div className="grid gap-4">
                        {assignments.data.map((assignment) => {
                            const assignmentBadge = getAssignmentBadgeProps(assignment.status);
                            const pastDue = assignment.due_date && isPastDue(assignment.due_date) && assignment.status !== 'completed';
                                ...
                                <Card key={assignment.id}>
                                    <CardHeader className="flex flex-row items-start justify-between space-y-0 pb-3">
                                        ...
                                        <div className="flex items-center gap-2">
                                            <Badge
                                                variant={portfolioStatusBadgeVariant[assignment.portfolio.status] ?? 'outline'}
                                                className={portfolioStatusBadgeClassName[assignment.portfolio.status] ?? ''}
                                            >
                                                {formatStatus(assignment.portfolio.status)}
                                            </Badge>
                                        ...
                                        <div className="flex items-center justify-between">
                                            <div className="text-muted-foreground flex items-center gap-4 text-sm">
                                                <span className="flex items-center gap-1">
                                                    ...
                                                    {assignment.portfolio.documents.length} {assignment.portfolio.documents.length === 1 ? 'document' : 'documents'}
                                                </span>
                                                {assignment.due_date && (
                                                    <span className={`flex items-center gap-1 ${pastDue ? 'font-medium text-destructive' : ''}`}>
                                                        {pastDue && <AlertTriangle className="h-3.5 w-3.5" />}
                        ...
                        <div className="flex gap-2">
                            {assignments.links[0]?.url ? (
                                <Button variant="outline" size="sm" asChild>
                                    <Link href={assignments.links[0].url}>&larr; Previous</Link>
                                </Button>
                            ) : (
                            ...
                            {assignments.links[assignments.links.length - 1]?.url ? (
                                <Button variant="outline" size="sm" asChild>
                                    <Link href={assignments.links[assignments.links.length - 1].url!}>Next &rarr;</Link>
                                </Button>
                            ) : (
                                <Button variant="outline" size="sm" disabled>
                                    Next &rarr;
